...
// Thread 3 to work with third object (using destructive read for ISO object)
def c3 = spawn { c4 ->
...
// Destructive read to send the ISO object to the thread
myIsolated1 := (myIsolated1 := myIsolated2) // Move `myIsolated1` to `myIsolated2`

// Send different objects to different threads
c1.send(myImmutable1)
c2.send(myLocal1)
c3.send(myIsolated2) // Send moved ISO object
